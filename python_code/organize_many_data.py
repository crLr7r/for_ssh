...
if __name__ == '__main__':
    ...
    def get_xi(variation_values, var_param):
        ...
        for value in variation_values:
            ...
            with open(f"{ORGANIZED_DIR}/1D_density/{model.filename}(state#{cstate}).txt", "w") as f:
                f.write(f"path: {path}\n{'site#':<10}{'Density':<35}\n")
            ...
            with open(f"{ORGANIZED_DIR}/1D_density_fit/{model.filename}(state#{cstate}).txt", "w") as f:
                f.write(f"xi={xi}\n")
                f.write(f"path: {path}\n{'site#':<10}{'Density':<35}\n")
    ...
    if graph == 'band_gap':
        ...
        with open(f"{ORGANIZED_DIR}/Bandgap_size/Diamond band gap(size=[{size_bounds[0],size_bounds[1]}], count={size_count}).txt", "w") as f:
    ...
    if graph == 'xi':
        ...
        with open(f"{ORGANIZED_DIR}/xi_lda/Diamond xi(lda=[0.1x2^{lda_power_bounds[0]}, 0.1x2^{lda_power_bounds[1]}]).txt", "w") as f:
        ...
        with open(f"{ORGANIZED_DIR}/xi_lda_fit/Diamond xi fit(lda=[0.1x2^{lda_power_bounds[0]}, 0.1x2^{lda_power_bounds[1]}]).txt", "w") as f:
